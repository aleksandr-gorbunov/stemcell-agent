...
4) audit-users
   ...
   Example:
   python skills/sec_analytics/tools/analyze.py audit-users --start 2026-04-15T00:00:00Z --end 2026-04-17T00:00:00Z
...
6) dns-tunnel
   Detect hosts with DNS tunneling indicators (high volume to a single domain with random-looking subdomain labels, high NXDOMAIN/TXT).
   Example:
...
7) investigate --user <email>
   ...
   Authentication-side signals (primary):
   - outside-EU successful login(s) (by country),
   - OR successful login(s) from public IPs outside known office NAT ranges,
   - impossible travel between consecutive successes (<6h between different countries),
   - sudden user-agent name change across successes,
   - burst of failures (>=5 within 15 minutes) preceding a success.

   Post-login anomalies (supporting, gated):
   - large external egress from internal hosts,
   - DNS tunneling indicators.
   These are ONLY considered if at least one authentication-side signal is present.

   Output JSON contract:
   {
     "user": "<email>",
     "compromise_likelihood": "high|medium|low",
     "evidence": ["string", ...]
   }
...
- EU country list is sourced from environment.yaml key `eu_countries` (includes EEA/CH as documented in KNOWLEDGE.md).
- Office NAT ranges are taken from allowlists.offices_nat_ranges.
- Thresholds are conservative and can be tuned as needed.
- Future: add host linkage by office NAT mapping and a --post-window minutes parameter before considering post-login anomalies.
