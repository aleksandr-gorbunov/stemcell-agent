...
def cmd_investigate(args):
    ...
    office_nat_ranges = allow.get("offices_nat_ranges", [])
    ...
    # Countries and non-office IP successes
    countries = []
    non_office_successes = []
    for h in successes:
        src_ip = h["_source"].get("source.ip")
        c = h["_source"].get("source.geo.country")
        if c:
            countries.append(c)
        if src_ip and office_nat_ranges and (not ip_in_any_range(src_ip, office_nat_ranges)):
            non_office_successes.append({"ip": src_ip, "ts": h["_source"].get("@timestamp")})
    uniq_countries = sorted(set([c for c in countries if c]))
    ...
    # Failure burst before a success (>=5 failures within 15 minutes preceding any success)
    failure_bursts = []
    fail_events = [h for h in auth_hits if h["_source"].get("event.outcome") == "failure"]
    for s in successes:
        ts_s = parse_iso(s["_source"]["@timestamp"])
        window_start = ts_s - timedelta(minutes=15)
        fails_in_window = [f for f in fail_events if window_start <= parse_iso(f["_source"]["@timestamp"]) < ts_s]
        if len(fails_in_window) >= 5:
            failure_bursts.append({"count": len(fails_in_window), "before": s["_source"]["@timestamp"]})

    # 2) Post-login anomalies (only considered if an auth-side red flag exists)
    consider_post = bool(outside_eu or impossible or ua_changed or failure_bursts or non_office_successes)
    heavy = []
    dns_sus = []
    if consider_post:
        body_exf = {
    ...
    for nos in non_office_successes:
        evidence.append(f"Success from non-office IP {nos['ip']} at {nos['ts']}")
    ...
    for fb in failure_bursts:
        evidence.append(f"{fb['count']} failures within 15m before success at {fb['before']}")
    if consider_post:
        for b in heavy:
    ...
    if non_office_successes: score += 2
    if impossible: score += 2
    if ua_changed: score += 1
    if failure_bursts: score += 1
    if consider_post and heavy: score += 1
    if consider_post and dns_sus: score += 1
