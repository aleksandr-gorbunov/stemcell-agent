...
- Authentication-side signals are primary. Post-login anomalies are now gated: only considered if at least one auth-side indicator exists, to avoid attributing environment-wide noise to a specific user.
...
- Skill: sec_analytics — multi-command analyzer (tools/analyze.py) implementing:
  - incidents, health-partner, health-cicd, audit-users, lateral, dns-tunnel, investigate (now gates post-login anomalies behind auth-side indicators).
...
## Next improvements
- Add office NAT → workstation subnet mapping and a --post-window parameter to tie internal hosts to a user session before considering post-login anomalies.
