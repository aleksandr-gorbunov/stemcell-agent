...
7) investigate --user <email>
   ...
   - burst of failures immediately followed by a success

   Post-login anomalies (supporting, gated):
   - large external egress from internal hosts
   - DNS tunneling indicators
   These are ONLY considered if at least one authentication-side signal is present. Otherwise they are ignored for this user to avoid unrelated attribution.
...
- Future: add host linkage by office NAT mapping and time window (--post-window minutes) before considering post-login anomalies.
