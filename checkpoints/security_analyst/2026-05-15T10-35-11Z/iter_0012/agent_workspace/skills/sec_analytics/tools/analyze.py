...
def ip_in_range(ip_str, start_end):
    # start_end like "a.b.c.d-w.x.y.z"
...
def cmd_health_cicd(args):
    ...
    for day in daterange_days(start, end):
        ...
        ok = to_app and to_db and to_ext if expected else True  # if not expected, don't fail health
...
def cmd_audit_users(args):
    ...
    for b in resp.get("aggregations", {}).get("by_user", {}).get("buckets", []):
        ...
        # Determine if outside EU
        outside_eu = [c for c in countries if c not in eu]
        if not outside_eu:
            continue
        # Exceptions
        weekdays = set()
        # We need days of week for CFO; approximate by reading first/last day only => fall back to fetching hits if needed
        if user == "cfo@example.com":
            # Fetch days for CFO successes
            body_hits = {
                "query": {"bool": {"filter": [
                    {"range": {"@timestamp": {"gte": args['--start'], "lt": args['--end']}}},
                    {"term": {"event.action": "login_attempt"}},
        ...
        if user in allowed_sales:
            ...
            continue
        # Default: outside EU -> investigate
...
def cmd_dns_tunnel(args):
    ...
    for b in resp.get("aggregations", {}).get("by_src", {}).get("buckets", []):
        ...
        # Find dominant base domain by grouping by second-level domain
        counts = {}
        subdomains = {}
...
def cmd_lateral(args):
    ...
    for b in resp.get("aggregations", {}).get("by_src", {}).get("buckets", []):
        ...
        if src == internal.get("cicd_host"):
            continue
        if uniq >= 15:  # heuristic threshold
...
def cmd_investigate(args):
    ...
    failures = [h for h in auth_hits if h["_source"].get("event.outcome") == "failure"]
    countries = sorted({h["_source"].get("source.geo.country") for h in successes if h["_source"].get("source.geo.country")})
    outside_eu = [c for c in countries if c and c not in eu]
    # 2) Look for post-login unusual network
    body_net = {
        "query": {"bool": {"filter": [
            {"range": {"@timestamp": {"gte": args['--start'], "lt": args['--end']}}},
            {"term": {"event.action": "allow"}},
        ...
        "size": 0,
        "aggs": {"by_src": {"terms": {"field": "source.ip", "size": 500}, "aggs": {"bytes_out": {"sum": {"field": "network.bytes_out"}}, "uniq_d": {"cardinality": {"field": "destination.ip"}}}}}
    }
    net = search(idx["network"], body_net, base).get("aggregations", {}).get("by_src", {}).get("buckets", [])
    heavy = [b for b in net if b.get("bytes_out", {}).get("value", 0) > 10_000_000]
    # Simple scoring
    score = 0
    reasons = []
    if outside_eu:
        score += 2; reasons.append(f"outside EU login countries: {outside_eu}")
    # burst of failures then success
    for i, h in enumerate(auth_hits[:-1]):
        if h["_source"].get("event.outcome") == "failure" and auth_hits[i+1]["_source"].get("event.outcome") == "success":
            score += 1; reasons.append("failure burst followed by success")
            break
    if heavy:
        score += 1; reasons.append("unusual network bytes after login window")
    sev = "low"
    if score >= 3: sev = "high"
    elif score == 2: sev = "medium"
    print(json.dumps({"user": user, "severity": sev, "reasons": reasons, "countries": countries}))


def cmd_incidents(args):
    # Compose from other detections
    base, idx, eu, allow, internal = load_env()
    results = []
    # 1) Credential stuffing excluding Sunday window and pen test IP
    body_burst = {
        "query": {"bool": {"filter": [
            {"range": {"@timestamp": {"gte": args['--start'], "lt": args['--end']}}},
            {"term": {"event.action": "login_attempt"}},
    ...
    # 2) Data exfil: top bytes_out to external IPs excluding CI/CD window/host
    body_exf = {
        "query": {"bool": {"filter": [
            {"range": {"@timestamp": {"gte": args['--start'], "lt": args['--end']}}},
            {"term": {"event.action": "allow"}},
    ...
    # 3) Lateral movement
    # reuse logic
    args_lat = {"--start": args['--start'], "--end": args['--end']}
    # Inline query again for independence
    body_lat = {
        "query": {"bool": {"filter": [
            {"range": {"@timestamp": {"gte": args['--start'], "lt": args['--end']}}},
    ...
    # 4) DNS tunneling
    # Shallow pass: high volume to single domain with many subdomains
    body_dns = {
        "query": {"range": {"@timestamp": {"gte": args['--start'], "lt": args['--end']}}},
        "size": 0,
    ...
    for b in dns:
        ...
        if counts[top_dom] >= 500 and len(subs.get(top_dom, [])) >= 50:
            results.append({"type": "dns_tunneling", "indicator": src, "evidence": f"{counts[top_dom]} queries to {top_dom} with {len(subs.get(top_dom, []))} unique subdomains"})
...
def main():
    ...
    if cmd in ("incidents","health-partner","health-cicd","audit-users","lateral","dns-tunnel","investigate"):
        # ensure time window for those that need it
        if cmd != "investigate" or True:
            # all commands require start/end except investigate additionally needs --user
