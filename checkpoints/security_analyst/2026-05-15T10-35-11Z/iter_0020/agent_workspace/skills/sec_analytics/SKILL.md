...
7) investigate --user <email>
   Investigate a single account for compromise indicators. Heuristics:
   - outside-EU successful login(s)
   - impossible travel between consecutive successes (<6h between different countries)
   - sudden user-agent name change
   - post-login anomalies within the window: large external bytes from internal hosts; DNS tunneling indicators

   Output JSON contract (used by tasks train_compromise_*):
   {
     "user": "<email>",
