# Security Analyst agent  understanding (rev 9)
...
- Corporate VPN egress: treat 203.0.113.0/24 and 198.51.100.0/24 as allowlisted; users whose nonEU logins are only through these IP ranges are excluded from audit hits.

Heuristics
- Inclusion rule for audit_user_access: include a user if any nonEU success during the window is not fully covered by an exception spanning both start and end of the window and not entirely via VPN egress ranges.
- Compromise scoring (investigate_account): +1 nonEU geo; +1 3 failures with 1 success; +1 3 offhours hourly buckets; +1 internal fanout 540 unique dests; +1 DNS longlabel volume 100. Verdict: 01 benign, 2 suspicious, 3 compromised.
- Time bounds: 3D start and < end.
